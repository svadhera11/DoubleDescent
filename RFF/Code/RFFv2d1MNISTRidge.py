...
gamma = 0.02
print(f"Using RBF gamma: {gamma:.5f}")
print(f"Using tiny Ridge Alpha (jitter): {RIDGE_ALPHA} (to approximate inversion)")

def compute_weights_exact(X, Y, alpha=1e-6):
    N, D = X.shape
    I_size = D if N > D else N
...
for n_components in COMPONENT_RANGE:
    ...
    rff_sampler = RBFSampler(gamma=gamma, n_components=n_components, random_state=42)
...
csv_filename = "nsamp=10000_nlvl=20per_gamma=d02_mse.csv"
...
plt.title(f'0-1 Loss (Linear Scale)\nNoise: {NOISE_LEVEL*100}%')
plt.legend()
...
plt.title(f'MSE Loss (Log Scale)\nNoise: {NOISE_LEVEL*100}%')
plt.legend()
